...
void copystring(char *dst, const char *src, int n) {
  while(n-- > 0 && *src)
    *dst++ = *src++;
  *dst = '\0';
}
...
void addCommand(const char *cmd) {
    copystring(history[history_count % HISTORY_SIZE], cmd, CMD_MAX);
    history_count++;
}
...
main(void)
{
  ...
  while(getcmd(buf, sizeof(buf)) >= 0){
    ...
    addCommand(cmd); // Save to history
